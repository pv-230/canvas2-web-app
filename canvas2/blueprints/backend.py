...
def add_course():
    ...
    # NOTE: yes, this is kind of pointless because it uses the unsafe session 'id' variable
    #       for the lookup. however at the bare minimum, 94-bit user IDs are harder to spoof
    #       than incrementing a single 'role' session integer, so i'm keeping it for now. -A
    userdata = db_conn.db.users.find_one(
    ...
    # NOTE: unsure if we'll want to implement this atm, because the same course can exist across
    #       multiple semesters. will still leave this reminder here for now tho. -A
    ...
    return redirect(url_for("frontend.course_page", code=new_class.inserted_id))
...
def submit_assignment():
    ...
    # NOTE: assignment["class"] is already of form ObjectID, so no need to convert
    enrollment = db_conn.db.enrollments.find_one({
